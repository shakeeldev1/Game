...
import { FaRocket, FaGem, FaBook, FaLightbulb } from 'react-icons/fa';
  ...
  return (
    <div className="w-full max-w-md mx-auto min-h-screen p-4 pb-24 particles-bg">
      ...
      <div className="text-center mb-8 animate-on-scroll">
        <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-[var(--color-neon-primary)] to-[var(--color-neon-accent)] rounded-2xl flex items-center justify-center shadow-[0_0_40px_rgba(240,165,0,0.4)] rotate-3 hover:rotate-0 transition-transform duration-500">
          <FaBook className="text-3xl text-[var(--color-neon-bg)]" />
        </div>
        <h1 className="text-3xl font-black gradient-text mb-2">Beginner's Guide</h1>
        <p className="text-[var(--color-neon-text)]/60 text-sm flex items-center justify-center gap-2">
      ...
      <div className="glass-card rounded-xl p-4 mb-6 animate-on-scroll">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-bold text-[var(--color-neon-text)]/60 uppercase tracking-wider">Your Progress</span>
          <span className="text-xs font-bold text-[var(--color-neon-primary)]">0/{guideSections.length}</span>
        </div>
        ...
        {guideSections.map((section, index) => (
          ...
          >
            ...
            <button
              ...
              className="w-full p-4 flex items-center justify-between hover:bg-[var(--color-neon-primary)]/5 transition-all duration-300 group"
            >
              <div className="flex items-center gap-4">
                <div className="relative">
                  ...
                  <div className="absolute -top-1 -right-1 w-5 h-5 bg-[var(--color-neon-bg)] rounded-full flex items-center justify-center text-[10px] font-bold text-[var(--color-neon-primary)] border border-[var(--color-neon-primary)]/30">
                    {section.id}
                  </div>
                </div>
                <div className="text-left">
                  <h3 className="font-bold text-[var(--color-neon-text)] group-hover:text-[var(--color-neon-primary)] transition-colors">{section.title}</h3>
                  <p className="text-[10px] text-[var(--color-neon-text)]/40 uppercase tracking-wider">{section.content.length} steps</p>
                </div>
              </div>
              <div className={`w-8 h-8 rounded-full bg-[var(--color-neon-primary)]/10 flex items-center justify-center transition-transform duration-300 ${expandedSections[section.id] ? 'rotate-180' : ''}`}>
                <FiChevronDown className="text-[var(--color-neon-primary)]" />
              </div>
            </button>
                ...
                <ul className="space-y-2">
                  {section.content.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-[var(--color-neon-text)]/80 text-sm group">
                      <div className="w-5 h-5 rounded-full bg-[var(--color-neon-primary)]/20 flex items-center justify-center flex-shrink-0 mt-0.5 group-hover:bg-[var(--color-neon-primary)]/30 transition-colors">
                        <FiCheckCircle className="w-3 h-3 text-[var(--color-neon-primary)]" />
                      </div>
                      <span className="group-hover:text-[var(--color-neon-text)] transition-colors">{item}</span>
                    </li>
                  ))}
                </ul>
      ...
      {/* Quick Tips */}
      <div className="mt-8 gradient-border rounded-2xl p-6 animate-on-scroll">
        <h3 className="font-bold text-[var(--color-neon-primary)] text-lg mb-4 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={i} className="glass-card rounded-xl p-4 text-center card-hover-lift">
              ...
              <div className="text-[var(--color-neon-primary)] font-black text-lg">{item.title}</div>
              <div className="text-xs text-[var(--color-neon-text)]/50">{item.desc}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Help Button */}
      <div className="my-8 text-center animate-on-scroll">
        <p className="text-[var(--color-neon-text)]/60 mb-4 text-sm flex items-center justify-center gap-2">
          <FiHelpCircle /> Need more help?
        </p>
        <button className="btn-premium text-[var(--color-neon-bg)] px-8 py-4 rounded-xl font-bold tracking-wide neon-glow w-full flex items-center justify-center gap-2">
