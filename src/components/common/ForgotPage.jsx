...
const ForgotPage = () => {
  ...
  return (
    <div className="relative min-h-screen py-2 flex items-center justify-center px-4 overflow-hidden particles-bg">
      ...
      <div className="relative w-full max-w-sm animate-on-scroll">
        <div className="glass-card rounded-md p-2 relative overflow-hidden">
          ...
          <button
            onClick={() => navigate(-1)}
            className="p-2.5 rounded-md glass-card text-[var(--color-neon-text)]/60 hover:text-[var(--color-neon-primary)] mb-8 active:scale-90 transition-all"
          >
            <FaArrowLeft size={14} />
          </button>
          ...
          {!isSent ? (
            <>
              <div className="flex flex-col items-center text-center space-y-4 mb-2">
                <div className="w-10 h-10 bg-gradient-to-br from-[var(--color-neon-primary)] to-[var(--color-neon-accent)] rounded-md flex items-center justify-center shadow-[0_0_30px_rgba(240,165,0,0.4)] neon-glow">
                  <FaKey className="text-[14px] text-[var(--color-neon-bg)]" />
                </div>
                <div>
                  <h2 className="text-[14px] font-black text-[var(--color-neon-text)]">
              ...
              <div className="glass-card p-1.5 rounded-xl mb-2 flex gap-1">
                <button
                  ...
                  className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-md transition-all flex items-center justify-center gap-2 ${method === 'phone'
                      ? 'btn-premium text-[var(--color-neon-bg)]'
                      : 'text-[var(--color-neon-text)] hover:text-[var(--color-neon-text)]'
                    }`}
                >
                  <FiPhone /> Phone
                </button>
                <button
                  ...
                  className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all flex items-center justify-center gap-2 ${method === 'email'
                      ? 'btn-premium text-[var(--color-neon-bg)]'
              ...
              <form onSubmit={handleReset} className="space-y-3">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-[var(--color-neon-text)] uppercase tracking-widest ml-1">
                  ...
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-[var(--color-neon-accent)] group-focus-within:text-[var(--color-neon-primary)] transition-colors">
                      {method === 'phone' ? <FaPhoneAlt size={14} /> : <FaEnvelope size={12} />}
                    </div>
                    <input
                      type={method === 'phone' ? 'tel' : 'email'}
                      required
                      placeholder={method === 'phone' ? 'Enter phone number' : 'Enter email address'}
                      className="w-full pl-11 pr-4 py-2 bg-[var(--color-neon-bg)]/50 text-[var(--color-neon-text)] placeholder-[var(--color-neon-text)]/30 rounded-xl border-2 border-[var(--color-neon-accent)]/20 focus:border-[var(--color-neon-primary)]/50 focus:shadow-[0_0_20px_rgba(240,165,0,0.2)] transition-all outline-none font-medium text-sm"
                    />
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  className="w-full py-2 btn-premium text-[var(--color-neon-bg)] font-black text-[11px] uppercase tracking-[0.2em] rounded-md neon-glow flex items-center justify-center gap-2"
                >
              ...
              {/* Security Note */}
              <div className="mt-2 glass-card rounded-xl p-4 flex items-start gap-3">
